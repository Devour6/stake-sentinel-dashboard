  ...
  try {
    ...
    try {
      const fetchStakeAccounts = async (rpcUrl: string) => {
        ...
        if (!response.ok) {
          throw new Error(`Failed to fetch stake accounts for total: ${response.statusText}`);
        }
...
export const fetchOnchainStakeHistory = async (votePubkey: string): Promise<StakeHistoryItem[]> => {
  try {
    ...
    if (currentStake > 0) {
      ...
      // Create a pseudo-random but deterministic pattern based on validator pubkey
      ...
      for (let i = 1; i <= 20; i++) {
        ...
        const variation = Math.sin((currentEpoch - i) * 0.3 + seed * 0.1) * 0.05;
